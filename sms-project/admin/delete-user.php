    ...
    <div class="container-fluid">
        <?php if (isset($_GET['action'])) { ?>
            <div class="card">
                <div class="card-body" id="form-container">
                    <?php if ($_GET['action'] == 'add-new') { ?>
                        <!-- Your form for adding new user -->
                    <?php } elseif ($_GET['action'] == 'fee-payment') { ?>
                        <!-- Your form for fee payment -->
                    <?php } ?>
                </div>
            </div>
        <?php } else { ?>
                    ...
                    <div class="table-responsive bg-white">
                        <table class="table table-bordered" id="users-table" width="100%">
                            ...
                            <tbody>
                                ...
                                $result = mysqli_query($db_conn, "SELECT * FROM accounts");
                                while ($row = mysqli_fetch_assoc($result)) {
                                    echo "<tr>";
                                    echo "<td>" . $row['id'] . "</td>";
                                    echo "<td>" . $row['name'] . "</td>";
                                    echo "<td>" . $row['email'] . "</td>";
                                    echo "<td><a href='delete.php?id=" . $row['id'] . "&user=" . $_GET['user'] . "' onclick='return confirm(\"Are you sure you want to delete this item?\");'><i class='fa fa-trash'></i> Delete</a></td>";
                                    echo "</tr>";
                                }
                                ?>
                            </tbody>
